...
setup_permissions() {
    if [ "$(id -u)" = "0" ] && [ "$PUID" != "0" ]; then
        ...
        chown -R "${PUID}:${PGID}" /etc/nginx /var/lib/nginx /var/log/nginx
        chown "${PUID}:${PGID}" /run/nginx.pid /app /app/api.jar /app/tdlib /app/web /app/entrypoint.sh
        if [ -d /app/data ] && find /app/data -maxdepth 1 -not -user "${PUID}" -print -quit | grep -q .; then
